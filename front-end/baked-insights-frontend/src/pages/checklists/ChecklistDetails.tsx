...
import { ChecklistFileField } from '../../components/Checklist/ChecklistFileField';

export const ChecklistDetails: React.FC = () => {
    const navigate = useNavigate();

    // id: template-id; cid: checklist-id
    ...
    const saveItemFileHandler = (index: number) => {
        return (value_fpath : HTMLInputElement | string) => {
            flushSync(() => {
                setItems((prevItems: APIChecklistItem[]) => {
                    const newItems = [...prevItems];
                    newItems[index].value_fpath = value_fpath;
                    return newItems;
                });
            });
            updateChecklistItem(items[index]);
        ...
        <Fragment key={index}>
            ...
            <div>
                {["text", "sku", "lot-number"].includes(fields[index].data_type)
                    ? <input
                        ...
                        onInput={(e: any) => {setItems((prevItems: APIChecklistItem[]) => {
                            const newItems = [...prevItems];
                            if (["sku", "lot-number"].includes(fields[index].data_type)) {
                                newItems[index].value = e.target.value.toUpperCase();
                            } else {
                ...
                : ["boolean"].includes(fields[index].data_type)
                    ...
                    <ChecklistFileField
                        disabled={isSubmitted}
                        value={items[index].value_fpath}
                        onEdit={saveItemFileHandler(index)}
                    />
                </div>
            }
            ...
            {items[index].completed_at && 
                <p className="text-xs font-medium text-teal-600 px-1">
                    Updated at {new Date(items[index].completed_at.concat("Z")).toLocaleTimeString()}.
                </p>
            }
        </Fragment>
    );
    ...
    <>
        ...
        <input
            disabled={true}
            className="bg-white w-full text-left outline-none font-bold rounded-lg border border-white text-slate-500 p-1 mx-auto"
            type="text"
            value={title}
            placeholder="Untitled"
        />
        {isSubmitted ?
            <p className="text-xs text-left italic text-gray-500 pb-4 px-1">
                This checklist has been submitted and can no longer be modified.
            </p>
            : <p className="text-xs text-left italic text-gray-500 pb-4 px-1">
                This checklist is still in progress and has not been submitted yet.
            </p>
        }
            ...
            <div className="min-w-32 max-w-60 mx-auto">
                ...
                {!isSubmitted &&
                    <button
                        disabled={false}
                        type="submit"
                        className={`${false ? "bg-gray-300 text-white" : "bg-gray-700 hover:bg-gray-900 text-white"} w-full text-left py-2 px-4 border-none outline-none text-sm font-medium rounded-md h-9 mt-4`}
