...
import { useTemplateShares } from '../../hooks/useChecklists';
...
export const ShareTemplate: React.FC<ShareTemplateProps> = ({ templateID }) => {
    ...
    useEffect(() => {
        if (!isLoadingTemplateShares && templateShares) {
            let selected = [];
            for (var i=0; i < templateShares.length; i++) {
                const share = templateShares[i];
    ...
    }, [templateShares, isLoadingTemplateShares]);

    const onSave = () => {
        const shareWith = selectedOptions?.map((item) => item.value).sort();
        const prevShareWith = templateShares?.map(s => s.user_id).sort();
        if (shareWith && prevShareWith && (shareWith.join(",") != prevShareWith.join(","))) {
            updateTemplateShares(shareWith)
        }
    };
